...
Wynikiem pracy narzędzia jest konwerter LOLCODE do Pythona. Aplikacja przyjmuje kod .lol, a następnie generuje funkcjonalny i gotowy do samodzielnego uruchomienia kod .py, który zachowuje pełną logikę pierwotnego programu.
...
| `t_COMMENT`     | `BTW .*`  | Komentarz jednolinijkowy |
| `t_MULTILINE_COMMENT` | `OBTW[\s\S]*?TLDR` | Komentarz wielolinijkowy |
---
...
```bash
   pip install ply streamlit streamlit-ace
```
2. Uruchom aplikację:
```bash
   streamlit run gui.py
```
3. Wklej kod LOLCODE w lewym panelu, kliknij **Apply** w edytorze, następnie **Transpiluj**.
---
